...
| `bedrock` | `AWS_BEDROCK_API_KEY`, `AWS_BEARER_TOKEN_BEDROCK`, or `BEDROCK_API_KEY` | `AWS_BEDROCK_BASE_URL`, `BEDROCK_BASE_URL`, `AWS_BEDROCK_REGION`, `AWS_REGION`, or `AWS_DEFAULT_REGION` |
| `cerebras` | `CEREBRAS_API_KEY` | `CEREBRAS_BASE_URL` |
| `cloudflare` / `workers_ai` | `CLOUDFLARE_API_KEY`, `CLOUDFLARE_API_TOKEN`, or `CLOUDFLARE_AUTH_TOKEN` | `CLOUDFLARE_ACCOUNT_ID`, `CLOUDFLARE_WORKERS_AI_BASE_URL`, `CLOUDFLARE_AI_BASE_URL`, or `CLOUDFLARE_BASE_URL` |
| `cohere` | `COHERE_API_KEY` | `COHERE_BASE_URL` |
| `dashscope` / `qwen` | `DASHSCOPE_API_KEY`, `ALIBABA_API_KEY`, or `QWEN_API_KEY` | `DASHSCOPE_BASE_URL`, `ALIBABA_MODEL_STUDIO_BASE_URL`, or `QWEN_BASE_URL` |
...
| `minimax` | `MINIMAX_API_KEY` or `MINI_MAX_API_KEY` | `MINIMAX_BASE_URL` |
| `mistral` | `MISTRAL_API_KEY` | `MISTRAL_BASE_URL` |
| `moonshot` / `kimi` | `MOONSHOT_API_KEY` or `KIMI_API_KEY` | `MOONSHOT_BASE_URL` or `KIMI_BASE_URL` |
| `nebius` / `nebius_token_factory` | `NEBIUS_API_KEY` or `TOKEN_FACTORY_API_KEY` | `NEBIUS_BASE_URL` or `TOKEN_FACTORY_BASE_URL` |
| `nvidia` / `nvidia_nim` | `NVIDIA_API_KEY` | `NVIDIA_BASE_URL` |
| `openrouter` | `OPENROUTER_API_KEY` | `OPENROUTER_BASE_URL` |
| `perplexity` | `PERPLEXITY_API_KEY` | `PERPLEXITY_BASE_URL` |
| `poe` | `POE_API_KEY` | `POE_BASE_URL` |
| `qianfan` / `ernie` | `QIANFAN_API_KEY`, `BAIDU_QIANFAN_API_KEY`, or `BAIDU_API_KEY` | `QIANFAN_BASE_URL` or `BAIDU_QIANFAN_BASE_URL` |
| `sambanova` | `SAMBANOVA_API_KEY` | `SAMBANOVA_BASE_URL` or `SAMBANOVA_URL` |
...
| `stepfun` / `step` | `STEPFUN_API_KEY` or `STEP_API_KEY` | `STEPFUN_BASE_URL` or `STEP_BASE_URL` |
| `together` | `TOGETHER_API_KEY` | `TOGETHER_BASE_URL` |
| `vercel` / `vercel_ai_gateway` | `AI_GATEWAY_API_KEY`, `VERCEL_AI_GATEWAY_API_KEY`, or `VERCEL_OIDC_TOKEN` | `AI_GATEWAY_BASE_URL`, `VERCEL_AI_GATEWAY_BASE_URL`, or `VERCEL_BASE_URL` |
| `vertex` / `google_vertex` | `VERTEX_AI_ACCESS_TOKEN`, `GOOGLE_CLOUD_ACCESS_TOKEN`, `GOOGLE_OAUTH_ACCESS_TOKEN`, or `VERTEX_API_KEY` | `VERTEX_AI_BASE_URL`, `GOOGLE_VERTEX_BASE_URL`, `GOOGLE_CLOUD_PROJECT`, or `VERTEX_BASE_URL` |
| `volcengine` / `doubao` | `VOLCENGINE_API_KEY`, `ARK_API_KEY`, or `DOUBAO_API_KEY` | `VOLCENGINE_BASE_URL`, `ARK_BASE_URL`, or `DOUBAO_BASE_URL` |
